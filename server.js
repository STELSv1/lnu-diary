...
// ГОЛОВНИЙ КОНТРОЛЕР (Рендерить сторінку при першому вході)
app.get('/dashboard', async (req, res) => {
    ...
    // При першому завантаженні завжди показуємо "summary" (всі предмети)
    let summary = [];
    try {
    ...
    res.render('dashboard', { 
        user: req.user, userSubgroup, subjects, 
        summary, schedule, tasks, 
        currentTab: req.query.tab || 'grades'
    });
});
...
// НОВИЙ РОУТ: ОТРИМАННЯ ОЦІНОК БЕЗ ПЕРЕЗАВАНТАЖЕННЯ
...
// Інші API (Task, Subgroup) без змін
...
app.post('/tasks/add', async (req, res) => {
    if (!req.isAuthenticated()) return res.status(401).json({ error: 'Unauthorized' });
    if (req.body.deadline && new Date(req.body.deadline) < new Date(Date.now() - 60000)) return res.status(400).json({ error: 'Час не може бути в минулому!' });
    const db = require('./config/database');
    try {
